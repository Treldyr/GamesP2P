...
function setupPieces() {
    ...
    // Pièces noires (rangée 8 – fausses, donc FirstTryMove)
    ...
    // Pions noirs (rangée 7 – faux)
    cols.forEach((col, i) => {
        const id = `bp${i + 1}`;
        const className = `piece_black ${col}7`;
        pieces.push(`<img id="${id}" src="../images/bp.png" class="${className}" onclick="FirstTryMove(this.classList[1])">`);
    });

    // Pièces blanches (rangée 1 – vraies, donc handleClickSecretQueen)
    ...
    cols.forEach((col, i) => {
        ...
        pieces.push(`<img id="${id}" src="../images/wp.png" class="${className}" onclick="handleClickSecretQueen(this.id)">`);
    });
